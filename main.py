...
def checkType(data):
    ...
    if(data_temp.find('Transfer-Encoding: chunked')):
        return 0
    elif(data_temp.find('Content-Length')):
        return 1
...
def parallel_download():
    # parallel
    #p1 = threading.Thread(target=conn_down, args=(Url1,)).start()
    #p2 = threading.Thread(target=conn_down, args=(Url2,)).start()
    p3 = threading.Thread(target=conn_down, args=(Url3,)).start()
